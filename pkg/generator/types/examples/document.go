...
type DocumentBuilder struct {
	language               Language
	root                   *Object
	example                extensions.ReadmeExample
	exampleIndex           int
	stack                  []Stackable
	skipAttributes         bool
	oneOfIndex             int
...
func NewDocumentBuilder(example extensions.ReadmeExample, index int) *DocumentBuilder {
	root := NewObject("")
...
func (b *DocumentBuilder) OnAttribute(property string, attribute *jsonschema.Schema, _ *jsonschema.Schema, _ *schema.VisitContext) {
	if !b.skipAttributes && schema.IsAttribute(attribute) && !attribute.Deprecated {
		if value := b.getExampleValue(attribute); value != nil {
...
func (b *DocumentBuilder) OnArrayStart(property string, array *jsonschema.Schema, itemTypeIsObject bool) {
...
func (b *DocumentBuilder) OnOneOfStart(oneOf *jsonschema.Schema, _ *jsonschema.Schema, _ *schema.VisitContext) {
	...
	for key, expectedValue := range discriminators {
		if discriminatorProperty, ok := oneOf.Properties[key]; ok {
			actualValue := schema.GetConstantOrDefault(discriminatorProperty)
...
func (b *DocumentBuilder) getExampleValue(att *jsonschema.Schema) any {
	var value any
	def := schema.GetConstantOrDefault(att)
	if b.example.UseDefaults {
		value = def
		if value == nil && len(att.Examples) > b.exampleIndex {
			value = att.Examples[b.exampleIndex]
		}
	} else {
		if len(att.Examples) > b.exampleIndex {
			value = att.Examples[b.exampleIndex]
		}
		if value == nil {
			value = def
